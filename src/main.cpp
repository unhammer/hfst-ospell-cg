...
int main(int argc, char ** argv)
{
	try
	{
		cxxopts::Options options(argv[0], " - make spelling suggestions with analyses in CG format");

		options.add_options()
			("a,archive", "Speller archive (Voikko zhfst format)", cxxopts::value<std::string>(), "BIN")
		...
		{
			std::cout << options.help({""}) << std::endl;
			std::cerr << "ERROR: expected archive as argument." << std::endl;
			return(EXIT_FAILURE);
		}
		...
		if(verbose) {
			std::cerr << "Reading transducer " << zhfstfile << std::endl;
		}
